...
export async function getLyrics(track) {
    logInfo(track.name + ' - ' + track.artists.toString())
    let lyrics = await find_lyrics(track.name + ' - ' + track.artists.toString())
    if (lyrics instanceof Error) {
        logInfo('deu errado')
        console.error(lyrics);
        lyrics = ``;
    } else {
        logInfo('Letra encontrada!')
        logInfo(lyrics)
...
const getID = url => {
    ...
    splits.push(splits[splits.length - 1].split('?')[0]);
