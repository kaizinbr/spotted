...
export async function extractTracks(trackIds) {
    ...
    const chunkedTracks = chunk(trackIds, 20);    
    ...
    for (let x = 0; x < chunkedTracks.length; x++) {
        ...
        // tracks[0].unsyncedlyrics = '' //await getLyrics(tracks)
        // console.log(tracks[0].external_ids.isrc)
...
const parseTrack = track => {
    // const lyrics = async () => ( getLyrics(track));
    // console.log(track)
    return {
        name: track.name,
        ...
        www: track.external_urls.spotify,
        uploaded_by: 'kaizin <3',
        encoded_by: 'YouTube ©',
        comment: 'url'
    };
};
...
export async function extractPlaylist(playlistId) {
    ...
    return {
        ...
        main_name: playlistInfo.name,
        owner: playlistInfo.owner.display_name,
        items: tracks
            .filter(item => item.track)
            .map(item => parseTrack(item.track)),
        image: playlistInfo.images.map(image => image.url)[0],
    };
}
...
export async function extractAlbum(albumId) {
    ...
    return {
        name: `${albumInfo.name} - ${albumInfo.artists[0].name}`,
        main_name: albumInfo.name,
        owner: albumInfo.artists[0].name,
        artists: albumInfo.artists.map(artist => artist.name),
        items: trackParsed,
        image: albumInfo.images.map(image => image.url)[0],
    };
}
